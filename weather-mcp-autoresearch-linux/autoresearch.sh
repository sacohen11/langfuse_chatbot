...
if ! "$PYTHON_BIN" evaluate_tool_spec.py --spec-file weather_tool_candidate.json --publish --result-file "$RESULT_FILE" > "$OUTPUT_FILE" 2>&1; then
  cat "$OUTPUT_FILE"
  exit 1
fi
...
for name in ("invocation_accuracy", "input_schema_quality", "output_schema_quality", "description_clarity", "conciseness"):
