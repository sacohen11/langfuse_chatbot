...
def main() -> None:
    parser = argparse.ArgumentParser(description="Save the best scored autoresearch attempt as the newest Langfuse prompt version.")
    ...
    prompt = langfuse.create_prompt(
        ...
        type="chat",
        prompt=[
            {"role": "system", "content": best["prompt_text"]},
            {"role": "user", "content": "{{customer_message}}"},
        ],
        labels=labels,
        tags=["sam-c-system-prompt", "autoresearch", "autoresearch-best", "support-triage"],
        config={
        ...
        commit_message=f"Save best autoresearch attempt: overall_score={best['overall_score']:.4f}",
    )
    langfuse.flush()
    print(
        json.dumps(
