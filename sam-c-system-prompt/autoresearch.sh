...
if ! "$PYTHON_BIN" evaluate_prompt.py --prompt-file prompt_candidate.txt --publish --result-file "$RESULT_FILE" > "$OUTPUT_FILE" 2>&1; then
  cat "$OUTPUT_FILE"
  exit 1
fi
...
for name in ("task_success", "tone", "safety", "conciseness"):
